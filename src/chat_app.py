...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async def run_customer_loyalty_task(customer_id):
        ...
        with tracer.start_as_current_span("Run Customer Loyalty Thread"):
            ...
            if not customer_loyalty_id:
                session_loyalty_response = {
                    "answer": "Customer loyalty agent not configured.",
    ...
    try:
        while True:
            ...
            #         fast_json_dumps({"answer": response, "agent": "single", "cart": persistent_cart})
            #     )
            # except Exception as e:
            ...
            try:
                ...
                with tracer.start_as_current_span("Handoff Intent Classification"):
                    ...
                    intent_result = handoff_service.classify_intent(
                        user_message=user_message, session_id=session_id, chat_history=formatted_history
                    )
                ...
                await websocket.send_text(
                    fast_json_dumps(
                        {"answer": "Error during handoff classification", "error": str(e), "cart": persistent_cart}
                    )
                )
                continue
            ...
            try:
                ...
                if image_url:
                    ...
                    await websocket.send_text(
                        fast_json_dumps({"answer": analysis_msg, "agent": agent_name, "cart": persistent_cart})
                    )
                ...
                if agent_name in ["interior_designer", "interior_designer_create_image", "cora"]:
                    ...
                    log_timing(
                        "Product Recommendations", product_start_time, f"Found: {len(products) if products else 0}"
                    )
                ...
                with tracer.start_as_current_span(f"{agent_name.title()} Agent Call"):
                    ...
                    if agent_name == "interior_designer_create_image":
                        ...
                        await websocket.send_text(
                            fast_json_dumps(
                                {"answer": thank_you_msg, "agent": "interior_designer", "cart": persistent_cart}
                            )
                        )
            ...
            # =============================================================================
                ...
                try:
                    await websocket.send_text(
                        fast_json_dumps({"answer": "Internal server error", "error": str(e), "cart": persistent_cart})
                    )
                except Exception:
